...
class PictureGameViewController: UIViewController, SFSpeechRecognizerDelegate {
...
*/
	...
	@objc func generateImage(){
		...
		//now we are setting it to add the letter after 10 seconds, take away the image after 20
		self.hideTimer = Timer.scheduledTimer(timeInterval: 10, target: self, selector: #selector(addLetter), userInfo: nil, repeats: false)
	}
	...
	@objc func addLetter(){
		...
		self.hideTimer = Timer.scheduledTimer(timeInterval: 10, target: self, selector: #selector(endOfTimer), userInfo: nil, repeats: false)
